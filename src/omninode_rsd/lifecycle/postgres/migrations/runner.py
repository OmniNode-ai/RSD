...
_LEDGER_RELATION_SQL: Final[str] = (
    "SELECT to_regclass('rsd_canary.schema_migrations')::text AS relation_name;"
)
_LEDGER_RELATION_NAME: Final[str] = "rsd_canary.schema_migrations"
...
class PostgresLifecycleMigrationRunner:
    ...
    def _read_applied_migrations(
        ...
        relation_row = connection.execute(_LEDGER_RELATION_SQL).fetchone()
        ...
        if relation_name is None:
            return ()
        if type(relation_name) is not str or relation_name != _LEDGER_RELATION_NAME:
